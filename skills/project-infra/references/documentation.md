...
| CONTEXT.md      | Optional: the domain glossary, each term with the wording to avoid, so code, documentation, and agents name one thing one way                                                              |
| PRODUCT.md      | Optional: the audience, register, and design principles a README and a documentation site's voice are checked against                                                                      |
| llms.txt        | Optional: the index an agent fetches first; generated by the documentation site build where one exists, hand-written and checked against the published surface otherwise                   |
| Release notes   | Optional: the narrative a generated changelog cannot carry, handled with [the release configuration](ci-and-releases.md#configure-release-please)                                          |
| Provenance file | Optional: where forked or imported upstream code (NOTICE, UPSTREAM.md) and third-party marks (THIRD_PARTY_NOTICES.md) came from, with the revision or hash that makes each claim checkable |
...
Each optional document earns its place through one check: a glossary holds only
the terms the code and the public interface actually use, an llms.txt inventory
matches the published commands, packages, and pages, and a curated release note
repeats only what a committed document already states. Where such a note and the
document behind it disagree, the document is correct and the note changes.
...
exist. An Ardo site emits `llms.txt` from its own content, which is the
generated case the document table above names.
